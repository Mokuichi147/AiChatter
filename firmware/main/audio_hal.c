...
#include "driver/i2c.h"        /* 旧I2C API (M5PM1読み取り互換) */
...
/* --------------------------------------------------------
 * I2C 旧API ヘルパー関数
 * M5PM1 (0x6E) は ESP-IDF 5.x 新API (i2c_master) の
 * transmit_receive / receive が ESP_ERR_INVALID_STATE になるため
 * 旧API (i2c_cmd_link) を使用する。M5Unifiedと同じ方式。
 * -------------------------------------------------------- */
...
static void i2c_bus_init(void) {
    ...
    ESP_LOGI(TAG, "I2Cバス初期化完了 (旧API, SDA=%d, SCL=%d)",
...
/* --------------------------------------------------------
 * PMIC (M5PM1, 0x6E) レジスタ書き込みラッパー
 * -------------------------------------------------------- */
static esp_err_t pmic_write_reg(uint8_t reg, uint8_t val) {
    esp_err_t ret = i2c_write_reg(PMIC_I2C_ADDR, reg, val);
    if (ret != ESP_OK) {
...
void pmic_lcd_power_on(void) {
    if (s_i2c_initialized) return;  /* 既に初期化済み */

    i2c_bus_init();

    /* I2Cアイドルスリープ無効化 */
    pmic_write_reg(0x09, 0x00);

    /* GPIO0,2,3をGPIO機能に (代替機能OFF) */
    pmic_write_reg(0x16, 0x00);

    /* GPIO2,3を出力モード、GPIO0を入力モード */
    pmic_write_reg(0x10, 0x0C);

    /* GPIO2,3をプッシュプルモード */
    pmic_write_reg(0x13, 0x00);

    /* GPIO2=HIGH(LCD電源ON), GPIO3=HIGH(AW8737/ES8311電源ON) */
    pmic_write_reg(0x11, 0x0C);
...
/* --------------------------------------------------------
 * ES8311 コーデック初期化 (旧I2C API)
 * pmic_lcd_power_on() で初期化済みのI2Cバスを再利用する
 * -------------------------------------------------------- */
static void codec_init(void) {
    ...
    for (size_t i = 0; i < sizeof(init_seq) / sizeof(init_seq[0]); i++) {
        esp_err_t ret = i2c_write_reg(ES8311_I2C_ADDR,
...
int pmic_read_reg(uint8_t reg, uint8_t *out, size_t len) {
    if (!s_i2c_initialized) return -1;
    esp_err_t ret = i2c_read_reg(PMIC_I2C_ADDR, reg, out, len);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "PMIC reg0x%02X 読み取り失敗: %s",
